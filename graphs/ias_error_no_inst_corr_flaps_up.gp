# gnuplot script file
# 
# Position Error - IAS error plot - Flaps UP, no correction for instrument error
#
## for direct inclusion in latex using epslatex terminal

load 'common_commands.gp'
set output '../graphs/ias_error_no_inst_corr_flaps_up.eps'

set title "Static Source Position Error - Airspeed - Flaps Retracted"
set grid xtics mxtics ytics mxtics lw 2, lw 1
set xrange [50:250]
set xlabel "IAS - instrument corrected (kt)"
set ylabel "Position Error in Airspeed (kt)"

# set label 10 "Flaps Retracted" at 125, -55 front center
set label 11 "CAS = IAS + Error" at 125, -2.75 front center
set label 12 "ASI Instrument Error Assumed to be Zero" at 125, -3.25 front center

# plot 'data/ias_err_no_inst_corr.txt' with lp lt -1 lw 2 pt 1 # lp = linespoints = lines joining points
plot 'data/ias_err_no_inst_corr.txt' with l lt -1 lw 2 # lp = linespoints = lines joining points
											# lt -1 = blackline
											# lw = line width
											# pt = point style
											# do gnuplot test to see options
