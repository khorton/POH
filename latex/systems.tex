...
{			\item \textbf{IGNITION --- MAG} --- Controls the single magneto.}
			...
			\item \textbf{IGNITION --- ELEC} --- Controls the Lightspeed electronic ignition. 
...
{shielded magneto ignition,}
 Lightspeed Engineering electronic ignition, fuel pump, fuel injection, and an alternate air induction system. 
 
\ifthenelse{\thePMAG = 1}
{The PMag has two power sources. It is powered from the Essential Bus during start and at ground idle. It is powered by its own internal generator if the rpm is above approximately 875 rpm, and will thus provide ignition even after a total electrical failure. The PMag internal generator may be checked by selecting the Electronic Ignition OFF and then pulling the PMAG CB, removing the Essential Bus power from the PMag, forcing it to use its internal alternator.}
{The magneto is self-powered, and will provide ignition even after a total electrical failure.} 
The Lightspeed Electronic ignition is powered from the Battery Bus, and will provide ignition as long as there is sufficient battery voltage available.
...
{The tachometer signal is from the P-Mag, and will continue to read if the P-Mag ignition is selected off, as long as the P-Mag unit has power. The RPM will read "0" if the P-Mag is inoperative.} 
\end{Note}
